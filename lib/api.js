...
export const getUserByDisplayName = async (displayName) => {
  ...
  if (displayName) {
    ...
    let q = query(userRef, where('displayName', '==', displayName));
    let res = await getDocs(q).catch(error => { console.log('getUserByDisplayName error:', error) });
    if (!res?.empty) {
      return res
    }
    return null
...
export const editDocAPI = async (articleId, docToEdit, body, replies) => {
  if (articleId, docToEdit && body) {
    if (docToEdit?.type === 'comment') {
      ...
      const newReply = {
        articleId, reply: body, createdAt: docToEdit?.createdAt, updatedAt: new Date(),
...
export const getResume = async (uid) => {
  if (uid) {
    const docRef = doc(db, 'users', uid, 'resume', 'resume1')
...
export const createResume = async (uid, data) => {
  try {
    if (uid) {
      const docRef = doc(db, 'users', uid, 'resume', 'resume1')
      docRef && await setDoc(docRef, data);
      return 'success'
    }
  } catch (error) {
    console.log(error)
...
export const updateResume = async (uid, data) => {
  try {
    if (uid) {
      const docRef = doc(db, 'users', uid, 'resume', 'resume1')
      docRef && await updateDoc(docRef, data);
      return 'success'
    }
  } catch (error) {
    console.log(error)
...
export const applyForJob = async (listerId, data) => {
  const docRef = collection(db, 'users', listerId, 'jobVancyResponses')
  await addDoc(docRef, data)
  return 'success'
}

export const hasCreatedCv = async (uid) => {
  if (uid) {
    const docRef = doc(db, 'users', uid, 'resume', 'resume1')
    const res = await getDoc(docRef)
    if (res.exists) {
