...
export const resizeSingleImage = (handleUpdatePics, requestType, photoURL, size, type) => {
    var files = photoURL;
    // var MAX_WIDTH = 96;
    var MAX_WIDTH = size;
    var imageFile = files;
        ...
        if (auth.currentUser) {
            ...
            reader.onload = function (event) {
                ...
                imgElement.onload = function (e) {
                    ...
                    handleUpdatePics(blob, fileName, type, contentType, requestType);
                };
            };
        }
    }
...
export const handleSwitchAccount = async (uid) => {
    ...
    // console.log('handleSwitchAccount - userRef', userRef)
    try {
        const userSnapshot = await getDoc(userRef)
        if (userSnapshot.exists) {
            // const encodePassword = window.btoa(password) // Encode String
            const decodedPassword = window.atob(userSnapshot.data().password) // Decode String
            // auth.
            await signInWithEmailAndPassword(auth, userSnapshot.data().email, decodedPassword).catch(error => { console.log(error) })
            await localStorage.removeItem("user");
            await UserProfile.setUser(userSnapshot.data()); // also set the auth current user to this new user
            return 'success'
        } else {
        ...
        if(error === `FirebaseError: Failed to get document because the client is offline.`){
            alert(`An error occured, make sure you're logged in`);
            return error;
        }
